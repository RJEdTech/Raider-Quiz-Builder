...
${items}
  ...
  function emitMC(q, id) {
    ...
    const respConditions = answers.filter(a => a.feedback).map(a => `
    ...
    const fbDisplay = (q.feedback.general ? `
      ...
      + (q.feedback.correct && correct ? `
        <respcondition continue="Yes"><conditionvar><varequal respident="${responseId}">${correct.id}</varequal></conditionvar><displayfeedback feedbacktype="Response" linkrefid="correct_fb"/></respcondition>` : '')
      + (q.feedback.incorrect && correct ? `
        <respcondition continue="Yes"><conditionvar><not><varequal respident="${responseId}">${correct.id}</varequal></not></conditionvar><displayfeedback feedbacktype="Response" linkrefid="general_incorrect_fb"/></respcondition>` : '');
    ...
    return `      <item ident="${itemId}" title="${escapeXml(q.title || 'Question')}">
        ...
        <resprocessing>
          <outcomes><decvar maxvalue="100" minvalue="0" varname="SCORE" vartype="Decimal"/></outcomes>${correctRC}${respConditions}${fbDisplay}
  ...
  /* =========================================================================
   * Manifest XML and ZIP bundling
   * ========================================================================= */

  function buildManifest(quizFilename, manifestId, resourceId) {
    return `<?xml version="1.0" encoding="UTF-8"?>
<manifest identifier="${manifestId}" xmlns="http://www.imsglobal.org/xsd/imsccv1p1/imscp_v1p1" xmlns:lom="http://ltsc.ieee.org/xsd/imsccv1p1/LOM/resource" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://www.imsglobal.org/xsd/imsccv1p1/imscp_v1p1 http://www.imsglobal.org/profile/cc/ccv1p1/ccv1p1_imscp_v1p2_v1p0.xsd http://ltsc.ieee.org/xsd/imsccv1p1/LOM/resource http://www.imsglobal.org/profile/cc/ccv1p1/LOM/ccv1p1_lomresource_v1p0.xsd">
  ...
  <organizations/>
  <resources>
    <resource identifier="${resourceId}" type="imsqti_xmlv1p2" href="${escapeXml(quizFilename)}">
      <file href="${escapeXml(quizFilename)}"/>
    </resource>
  </resources>
</manifest>`;
  }
  ...
  async function generate(parseResult) {
    ...
    const slug = (parsed.title || 'quiz').toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '') || 'quiz';
    const quizFilename = `${slug}.xml`;

    const idGen = makeIdGen();
    const manifestId = idGen();
    const resourceId = idGen();

    const zip = new JSZip();
    zip.file('imsmanifest.xml', buildManifest(quizFilename, manifestId, resourceId));
    zip.file(quizFilename, buildAssessmentXml(parsed));
  ...
  if (typeof module !== 'undefined' && module.exports) {
    module.exports = { parse, generate, buildAssessmentXml, buildManifest, normalize };
  }
